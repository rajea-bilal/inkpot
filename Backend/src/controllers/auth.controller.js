...
export async function resendEmailController(req, res) {
  try {
    ...
    // grab the details to send another email using nodemailer's sendEmail function

    const user = await userModel.findOne({ email });
    console.log("user found from the email", user);
    ...
    console.error("Error in resendEmailController:", err);
    res.status(400).json({
      message: "resend email unsuccessful",
      status: false,
      err: err.message,
    });
  }
